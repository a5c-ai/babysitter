...
| `@a5c-ai/hooks-adapter-genty` | `packages/adapters/hooks/adapter-genty` | 6.0.0 | (absent) | (absent) | no | package has never been published (FIX-005); the hooks CLI's exact pin is deliberately deferred until this package first publishes (`//deferred-dependency` note in `packages/adapters/hooks/cli/package.json`; `adapter-loader.ts` excludes `genty` from `KNOWN_ADAPTERS`, so the CLI does not consume it today) — added as a checklist step of the recovery runbook § 3.8 |
