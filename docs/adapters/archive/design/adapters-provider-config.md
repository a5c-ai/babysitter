...
This spec defines how `adapters` resolves which LLM provider and model a harness should use, covering both the CLI surface (`adapters launch`, `adapters run`) and the programmatic SDK. It introduces a **provider configuration system** that abstracts away per-harness differences in how providers and models are specified.

The core problem: each harness has its own way of configuring providers — Claude uses env vars (`ANTHROPIC_API_KEY`, `CLAUDE_CODE_USE_BEDROCK`), Codex uses `config.toml` profiles, Gemini uses settings.json and env vars, OpenCode uses `opencode.json` with Vercel AI SDK provider packages. `adapters` unifies this into a single configuration surface that is then translated into harness-specific configuration at launch time.
...
| `adapters-proxy` package | `docs/provider-mux.md` |
...
2. Global config:  ~/.adapters/providers.json
3. Project config: .adapters/providers.json
...
`~/.adapters/providers.json` and `.adapters/providers.json`:

```json
{
  "$schema": "https://a5c.ai/schemas/adapters-providers.json",
  "version": 1,
...
```typescript
configContent = `
[model_providers.adapters]
name = "adapters-proxy"
base_url = "${proxyBaseUrl}/v1/"
wire_api = "responses"
env_key = "ADAPTERS_PROXY_KEY"
`;
configPath = path.join(os.homedir(), '.codex', 'config.toml');
args = ['--profile', 'adapters'];
```
...
function buildOpenCodeConfig(resolved: ProviderConfig, proxyUrl?: string): string {
  ...
  return JSON.stringify({
    ...
    provider: {
      adapters: { npm, options: { baseURL: baseUrl } },
    },
    model: {
      default: `adapters/${resolved.model}`,
    },
  });
}
```
...
```typescript
import { resolveProvider, translateForHarness } from '@a5c-ai/comm-adapter';
...
`adapters models` is extended to show available models per provider:

```bash
# List models for a provider
adapters models --provider bedrock
adapters models --provider vertex --region us-central1

# List models available for a harness+provider combo
adapters models --harness claude --provider bedrock
```
...
`adapters` maintains a model ID translation table. When `--model` is given, it is translated to the provider-specific format. Users can also use the canonical format and let adapters translate.
...
Before launching, `adapters` validates:
...
For `provider: "ollama"`, adapters integrates with the local Ollama server:
...
adapters models --provider ollama
```

The `adapters-proxy` package handles Ollama model management (see `docs/provider-mux.md` §6).
...
- The proxy uses a random session-scoped bearer token for auth between adapters and the proxy
...
- `~/.adapters/providers.json` should be `0600` (user-only read/write)
- `adapters config` warns if permissions are too open
- Project-level `.adapters/providers.json` should not contain secrets (use env var references)
