...
| `verify:release-artifacts` | `node ./scripts/verify-release-artifacts.mjs` | No-model release gate | release-gate | Not invoked by name. `publish.yml` reaches the script through `verify_release_artifacts` → `release-artifact-verifier.yml`, which runs `node scripts/verify-release-artifacts.mjs --report-dir artifacts/release-verifier` strictly (`allow_known_failures: 'false'` on every lane) over the full inventory and uploads `release-verifier-reports`. **This job gates publication**: `prepare_staging_publish` declares it in both `needs:` and its `if:` expression, so every `publish_staging_*` job is transitively blocked on a green matrix; `publish-packages-from-tag.yml` runs its own `verify_release_artifacts` job against the release tag ref before `publish_non_plugin_packages`. |
...
| `test:binary-renames` | `node ./scripts/check-binary-renames.cjs` | No-model | contract | `ci.yml` job `test`, step "Binary rename guardrail" (FIX-004: proves every declared bin target, including the extensions compatibility bin, resolves to a tsconfig-derived emitted path). |
...
| `@a5c-ai/comm-adapter` | `test` | No-model | unit-or-integration | Adapter maintainers | `test-logs/adapters-core-pty-contract.log` (ci.yml), `adapter-logs/agent-comm-adapter-tests.log` (publish.yml) | publish.yml `validate_mux` and ci.yml `test` (step "adapters-core PTY contract (@a5c-ai/comm-adapter suite)"), both via `node scripts/adapters-build.cjs test packages/adapters/core`. The `test` script is `vitest run src tests`, so this is the lane that executes the FIX-009 PTY suites (`tests/pty.test.ts`, `tests/pty-consumer.packaged.test.ts`) — the ci.yml step is named for PTY so the coverage is greppable. |
...
| --- | --- | --- |
| `.github/workflows/ci.yml` | PR/push docs, package, adapter, coverage, SDK, and observer validation. Jobs: `docs-quality`, `test`, `gateway-node-engine` (the only matrix job: `node-version: ['22.13.0', '22']`), `workspace-coverage`, `observer-dashboard`. It is the **only** workflow that runs `test:release-tooling` under its own name (`release-artifact-verifier.yml` runs it as a step of the release gate). `test:babysitter-metapackage` runs here and in `publish.yml`'s `validate_core`. | Keep current jobs, then align uploaded logs with `test-logs/`, `coverage/`, `e2e/`, and `docs-qa/` names |
| `.github/workflows/publish.yml` | Unified branch-aware validation, live-stack preflight, publish, deploy, release-tag, and external-plugin sync ordering. Validation jobs: `lint`, `build_all`, `validate_core`, `validate_mux`, `validate_cloud`, `validate_kradle`, `validate_observer_and_compiler`, `e2e_integration`, `verify_release_artifacts`. `prepare_staging_publish` runs only when all of those report `success`. | Owns current no-model validation jobs plus the model-backed live-stack scenario/OS matrix before publish jobs |
...
| `.github/workflows/release-artifact-verifier.yml` (reusable) | Called by `publish.yml` job `verify_release_artifacts` and by `publish-packages-from-tag.yml`'s job of the same name (which passes `ref: <release tag>` so the tag's tree is what gets verified). Runs `test:release-tooling` and the full-inventory `verify-release-artifacts.mjs` sweep. `allow_known_failures` defaults to `'false'` on every lane — dispatch, `pull_request` and `workflow_call` — and both release callers pass `'false'` explicitly, so the sweep runs **without** `--allow-known-failures`. **It gates publication:** `publish.yml`'s `prepare_staging_publish` and `publish-packages-from-tag.yml`'s `publish_non_plugin_packages` both declare `verify_release_artifacts` in `needs`, so a packed-artifact failure stops every downstream publish job. Pinned by `scripts/__tests__/verify-release-artifacts.test.mjs`. | Release-gate lane; artifact `release-verifier-reports` |
...
These were listed above as unwired; they are now wired, and the row for each command in the tables reflects the current lanes.

- `test:babysitter-metapackage` — was `ci.yml` only. It now also runs in `publish.yml`'s `validate_core` (step `Metapackage shim tests (packed exit-code propagation)`, log `core-logs/babysitter-metapackage-tests.log`), so the publication path re-checks FIX-007 exit-code propagation against the packed metapackage.
- `verify_release_artifacts` — was in no publish job's `needs`, so it reported rather than gated. `publish.yml`'s `prepare_staging_publish` now needs it (in both `needs:` and its `if:`, which uses `!cancelled()`), and every `publish_staging_*` job hangs off that job. The tag-recovery workflow `publish-packages-from-tag.yml` did not call the verifier at all; it now has its own `verify_release_artifacts` job, gated on `should_publish`, passing `allow_known_failures: 'false'` and the release tag as `ref`, with `publish_non_plugin_packages` depending on it.
- The FIX-009 PTY suites (`packages/adapters/core/tests/pty.test.ts` and `tests/pty-consumer.packaged.test.ts`) ran only inside `publish.yml`'s `validate_mux`. `ci.yml` now runs the same suite at PR time under the greppable step name `adapters-core PTY contract (@a5c-ai/comm-adapter suite)` (log `test-logs/adapters-core-pty-contract.log`).
