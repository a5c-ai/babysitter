...
| External harness through agent-mux proxy | Model-backed | `adapters launch <harness> <provider> --with-proxy` or `--with-proxy-if-needed` | Real Codex/Claude-compatible harness traffic uses the local proxy URL, emits a redacted launch plan, and completes a sentinel stream |
...
| P14 | No-model | Pipeline-owned stack matrix across `agent-mux-mocks` and real-agent CLI shims for `claude`, `codex`, `pi`, and `gemini` | `adapters install --dry-run`, profile-backed launch/run, transport-mux mock-model request evidence, and optional hooks-mux normalized phase artifact from `no_model_mock_matrix` |
