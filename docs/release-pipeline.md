...
| Layer | File | Contract |
| --- | --- | --- |
| Inventory | `scripts/lib/publishable-packages.cjs` | `listPublishablePackages(repoRoot)` returns every **git-tracked** workspace manifest where `private !== true` **and** `publishConfig.access === "public"`, sorted by package name. It throws on a duplicate name or directory, an unreadable workspace manifest, or a tracked manifest missing from the working tree — the inventory never silently drops a package. Run it directly (`node scripts/lib/publishable-packages.cjs`) to print the inventory as JSON. |
