...
- Harness CLI entry: `packages/babysitter-harness/src/cli/main.ts`
...
### Babysitter Harness (`packages/babysitter-harness` / `@a5c-ai/babysitter-harness`)

```bash
npm run build --workspace=@a5c-ai/babysitter-harness    # tsc → dist/
npm run clean --workspace=@a5c-ai/babysitter-harness    # rimraf dist
npm run test --workspace=@a5c-ai/babysitter-harness     # vitest run (all tests)
```
...
| `packages/babysitter-harness` | `@a5c-ai/babysitter-harness` | Optional harness runtime CLI: Pi + agent-mux integration, session/cost/governance, daemon, observability. |
...
## Babysitter Harness Architecture (`packages/babysitter-harness/`)
