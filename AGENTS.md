...
- `ctx.breakpoint()` — human approval gate, returns `Promise<BreakpointResult>` (`{ approved, response?, feedback?, option?, respondedBy?, allResponses? }`). Supports routing: `expert` (string | string[]), `tags`, `strategy` (`'single'` | `'first-response-wins'` | `'collect-all'` | `'quorum'`), `previousFeedback`, `attempt`.
