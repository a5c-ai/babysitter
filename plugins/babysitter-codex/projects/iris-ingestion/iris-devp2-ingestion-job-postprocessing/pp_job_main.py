...
def main():
    ...
    if original_count != len(group_ids):
        print(f"[PP] Removed {original_count - len(group_ids)} duplicate group IDs")
    ...
    # Refresh dashboard stats after PP completes (activity timestamps are now updated)
    print(f"\n[PP] Refreshing dashboard stats...")
    try:
        refresh_dashboard_stats()
        print("[PP] Dashboard stats refreshed successfully")
    except Exception as e:
        print(f"[PP] WARNING: Dashboard refresh failed (non-fatal): {e}")

    sys.exit(0)


def refresh_dashboard_stats():
    """Refresh the IndexedContent table that powers the dashboard Active Groups widget.

    Uses direct SQL for performance — the ORM Exists subquery times out on large datasets.
    This runs after the activity_timestamp PP step has updated last_active_timestamp.
    """
    import psycopg

    # Reuse the same DB config logic as resolve_group_ids
    db_config_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), 'configs', 'db.json'
    )
    if not os.path.exists(db_config_path):
    ...
    try:
        ...
        # Get top 10 active groups (simple query — no expensive Exists subquery)
        cur.execute("""
            SELECT id, name, last_active_timestamp, last_active_timestamp_readable
            FROM entities
            WHERE status = '1' AND type = 'group' AND last_active_timestamp IS NOT NULL
        ...
        for g in groups:
            group_list.append({
                "id": g[0],
                "name": g[1],
                "last_active_timestamp": str(g[2]) if g[2] else None,
                "last_active_timestamp_readable": g[3],
            })

        # Get counts
        cur.execute("SELECT COUNT(*) FROM entities WHERE status = '1' AND type = 'group'")
        group_count = cur.fetchone()[0]

        cur.execute("SELECT COUNT(*) FROM entities WHERE status = '1' AND type NOT IN ('group', 'lid', 'im_account')")
        entity_count = cur.fetchone()[0]

        # Message count — use approximate from pg_class for speed
        cur.execute("""
            SELECT reltuples::bigint FROM pg_class WHERE relname = 'message_application_data'
        """)
        message_count = cur.fetchone()[0]

        # Read existing IndexedContent to preserve words data
        cur.execute("SELECT top_words FROM indexed_content WHERE id = 1")
        row = cur.fetchone()
        old_data = {}
        if row and row[0]:
            old_data = json.loads(row[0]) if isinstance(row[0], str) else row[0]
        ...
        content_data = json.dumps({
            ...
            "words": old_data.get("words", []),
            "groups": group_list,
        })

        # Upsert IndexedContent
        cur.execute("""
            UPDATE indexed_content SET top_words = %s, added_time = NOW() WHERE id = 1
        """, (content_data,))
        conn.commit()

        # Clear Django cache key
        try:
            cur.execute("DELETE FROM django_cache WHERE cache_key LIKE '%dashboard_stats%'")
            conn.commit()
        except Exception:
            pass  # Cache table may not exist or have different schema

        print(f"[PP] Dashboard: {group_count} groups, {entity_count} entities, ~{message_count} messages, {len(group_list)} active groups")
