...
Test-Path "$HOME/.codex/skills/babysitter-codex/.codex/turn-controller.js"
```
...
- let the installed harness assets handle the runtime loop and persisted state
- expect one orchestration phase per turn
- expect completion only when the run emits a `completionProof`

Low-level turn-control, result-posting, and SDK command details are internal to
the Codex harness. They live in the Codex skill and maintainer docs, not in
this install guide.
