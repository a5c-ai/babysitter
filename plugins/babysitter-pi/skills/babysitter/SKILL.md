...
- In the built-in internal `session:create` PI orchestration path, prefer the dedicated pending-effect tools: use `babysitter_run_shell_effect` for shell work and `babysitter_dispatch_effect_harness` for the other dispatchable effect kinds, then post via `babysitter_task_post_result`. Shell effects are still first-class pending effects and must not be skipped or assumed to be host-side automation.
