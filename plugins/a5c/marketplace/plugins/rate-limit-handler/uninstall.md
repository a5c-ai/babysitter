...
2. From `hooks.Notification`: Remove the entry whose command contains `rate-limit-handler`
3. From `hooks.PostToolUse`: Remove the entry whose command contains `rate-limit-handler`
...
- The retry log at `.claude/rate-limit-handler/retry.log` is removed with the plugin files
