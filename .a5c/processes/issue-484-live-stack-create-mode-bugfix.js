...
const publishTask = defineTask('issue-484.publish', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      ...
      ``,
      `Root cause: create-mode validation allowed run proof to substitute for the required generated .a5c/processes/odyssey-live-test.mjs artifact, and pi create-mode prompts were not routed through the Babysitter command surface.`,
      ``,
      `Verification: npm run build:sdk; HOME=<tmp> npm run test:sdk -- --runInBand; npm run verify:metadata; focused live-stack runner tests.`,
      `COMMENT`,
      `printf '\\nPR: %s\\n' "$PR_URL" >> "$COMMENT_BODY"`,
