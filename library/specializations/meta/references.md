# Meta Specialization References

## Babysitter SDK Documentation

### Core SDK
- **SDK Repository**: Internal Babysitter SDK documentation
- **Process Patterns**: `library/` - Example processes
- **Methodology Library**: `library/methodologies/`

### Process Definition Patterns
- **defineTask API**: Task definition with kind (agent, skill, node, shell, breakpoint)
- **ctx.task()**: Execute tasks within process flow
- **ctx.breakpoint()**: Human-in-the-loop approval gates
- **ctx.log()**: Structured logging for observability

## Claude Code Skills and Agents

### Official Resources
- **Claude Code Documentation**: https://docs.anthropic.com/claude/docs/claude-code
- **MCP Protocol**: https://modelcontextprotocol.io/
- **Claude Skills Format**: https://docs.anthropic.com/claude/docs/skills

### Community Skills Repositories
- **Awesome Claude Skills (Composio)**: https://github.com/ComposioHQ/awesome-claude-skills
- **Awesome Claude Skills (VoltAgent)**: https://github.com/VoltAgent/awesome-claude-skills
- **Claude Code Skills**: https://github.com/levnikolaevich/claude-code-skills
- **Claude Skills (alirezarezvani)**: https://github.com/alirezarezvani/claude-skills/tree/main
- **Claude Code Skill Factory**: https://github.com/alirezarezvani/claude-code-skill-factory
- **Claude Skills Marketplace**: https://github.com/mhattingpete/claude-skills-marketplace
- **Awesome Claude Code Plugins**: https://github.com/ccplugins/awesome-claude-code-plugins
- **Claude Code Plugins Marketplace**: https://github.com/ccplugins/marketplace

### Specialized Skill Collections
- **Trail of Bits Skills**: https://github.com/trailofbits/skills
- **AI Research Skills**: https://github.com/zechenzhangAGI/AI-research-SKILLs
- **AWS Agent Skills**: https://github.com/itsmostafa/aws-agent-skills
- **Terraform Skill**: https://github.com/antonbabenko/terraform-skill
- **Scientific Skills**: https://github.com/K-Dense-AI/claude-scientific-skills

### Agent Resources
- **Agent Resources Collection**: https://github.com/kasperjunge/agent-resources
- **SWE Agents**: https://github.com/wshobson/agents
- **Claude Research Team**: https://github.com/bigph00t/claude-research-team

### Plugin Examples
- **Compound Engineering Plugin**: https://github.com/EveryInc/compound-engineering-plugin
- **Spec Kit Subagent Plugin**: https://github.com/jcmrs/claude-code-spec-kit-subagent-plugin
- **Design Engineering Plugins**: https://github.com/agenisea/ai-design-engineering-cc-plugins
- **ODIN Claude Plugin**: https://github.com/OutlineDriven/odin-claude-plugin

## Process Design Methodologies

### Software Process Frameworks
- **Agile Manifesto**: https://agilemanifesto.org/
- **Scrum Guide**: https://scrumguides.org/
- **Kanban Guide**: https://kanban.university/kanban-guide/
- **SAFe Framework**: https://scaledagileframework.com/

### Quality Gate Patterns
- **Continuous Integration**: https://martinfowler.com/articles/continuousIntegration.html
- **Continuous Delivery**: https://continuousdelivery.com/
- **Test-Driven Development**: https://www.agilealliance.org/glossary/tdd/

### Workflow Orchestration
- **Temporal.io**: https://temporal.io/ - Workflow orchestration patterns
- **Apache Airflow**: https://airflow.apache.org/ - DAG-based workflows
- **Prefect**: https://www.prefect.io/ - Modern workflow orchestration

## Documentation Standards

### Technical Writing
- **Google Developer Documentation Style Guide**: https://developers.google.com/style
- **Microsoft Writing Style Guide**: https://docs.microsoft.com/en-us/style-guide/welcome/
- **Divio Documentation System**: https://documentation.divio.com/

### Markdown Standards
- **CommonMark Specification**: https://commonmark.org/
- **GitHub Flavored Markdown**: https://github.github.com/gfm/
- **Markdownlint Rules**: https://github.com/DavidAnson/markdownlint

### API Documentation
- **OpenAPI Specification**: https://spec.openapis.org/oas/latest.html
- **JSON Schema**: https://json-schema.org/

## Architecture Decision Records (ADRs)

### ADR Resources
- **ADR GitHub Organization**: https://adr.github.io/
- **Nygard ADR Article**: https://cognitect.com/blog/2011/11/15/documenting-architecture-decisions
- **MADR Template**: https://adr.github.io/madr/
- **ADR Tools**: https://github.com/npryce/adr-tools
- **Log4brains**: https://github.com/thomvaill/log4brains

## Prompt Engineering

### Resources
- **Anthropic Prompt Engineering Guide**: https://docs.anthropic.com/claude/docs/prompt-engineering
- **OpenAI Prompt Engineering**: https://platform.openai.com/docs/guides/prompt-engineering
- **Learn Prompting**: https://learnprompting.org/

### Best Practices
- **Chain of Thought Prompting**: https://arxiv.org/abs/2201.11903
- **ReAct Pattern**: https://arxiv.org/abs/2210.03629
- **Tree of Thoughts**: https://arxiv.org/abs/2305.10601

## JSON Schema

### Specification
- **JSON Schema Draft 2020-12**: https://json-schema.org/draft/2020-12/json-schema-core.html
- **JSON Schema Validation**: https://json-schema.org/draft/2020-12/json-schema-validation.html

### Tools
- **AJV Validator**: https://ajv.js.org/
- **JSON Schema Store**: https://www.schemastore.org/json/

## MCP (Model Context Protocol)

### Official Resources
- **MCP Specification**: https://modelcontextprotocol.io/
- **MCP SDK**: https://github.com/modelcontextprotocol/sdk

### MCP Server Collections
- **MCP Hub**: https://mcphub.io/
- **Awesome MCP Servers**: https://github.com/modelcontextprotocol/awesome-mcp-servers

## Version Control and Git

### Git Workflows
- **Git Flow**: https://nvie.com/posts/a-successful-git-branching-model/
- **GitHub Flow**: https://guides.github.com/introduction/flow/
- **Trunk Based Development**: https://trunkbaseddevelopment.com/

### Conventional Commits
- **Conventional Commits**: https://www.conventionalcommits.org/
- **Semantic Versioning**: https://semver.org/

## Testing and Quality

### Testing Frameworks
- **Jest**: https://jestjs.io/
- **Vitest**: https://vitest.dev/
- **Playwright**: https://playwright.dev/

### Code Quality
- **ESLint**: https://eslint.org/
- **Prettier**: https://prettier.io/
- **SonarQube**: https://www.sonarqube.org/

## Domain-Driven Design

### DDD Resources
- **Domain-Driven Design Reference**: https://www.domainlanguage.com/ddd/reference/
- **DDD Community**: https://www.dddcommunity.org/
- **Event Storming**: https://www.eventstorming.com/

## Knowledge Management

### Systems
- **Zettelkasten Method**: https://zettelkasten.de/
- **Second Brain (PARA)**: https://fortelabs.com/blog/para/
- **Obsidian**: https://obsidian.md/

## Automation and CI/CD

### GitHub Actions
- **GitHub Actions Documentation**: https://docs.github.com/en/actions
- **Awesome GitHub Actions**: https://github.com/sdras/awesome-actions

### npm/Node.js
- **npm Documentation**: https://docs.npmjs.com/
- **Node.js Best Practices**: https://github.com/goldbergyoni/nodebestpractices

---

## Quick Reference Links

| Category | Primary Resource |
|----------|------------------|
| Claude Skills | https://github.com/ComposioHQ/awesome-claude-skills |
| MCP Servers | https://mcphub.io/ |
| Process Patterns | Babysitter SDK internal docs |
| Documentation | https://developers.google.com/style |
| ADRs | https://adr.github.io/ |
| Prompt Engineering | https://docs.anthropic.com/claude/docs/prompt-engineering |
| JSON Schema | https://json-schema.org/ |
