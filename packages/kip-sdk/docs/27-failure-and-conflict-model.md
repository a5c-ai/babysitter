...
- **Layer ② (`proj`)** owns the *set-pure* outcomes: **proj-demotion/quarantine**, **`kip:conflict`**, and **pending/pin-incomplete**. All three are pure functions of the admitted set, so they are byte-identical across replicas (the SEC guarantee) and re-evaluate monotonically as facts arrive.
