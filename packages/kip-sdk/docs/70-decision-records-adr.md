...
**Decision.** Two tiers. **GUARANTEED** (no spawn, deterministic): isomorphic-git for the HEAD sha + tracked-file set, `node:fs` walk/read, regex import/require/export extraction, extension + shebang format detection, and newline LOC. **PROBED-and-skipped**: `rg`/`tokei`/`scc`/`cloc`/`ast-grep`/`tsc`/`eslint` — resolved via exported `resolveOnPath` (`ask.ts:221`), probed `<bin> --version` via `probeVersionOf` (`ask.ts:547`) returning a `HarnessCliProbe`, spawned via `buildHarnessSpawn` (Windows `.cmd` trampoline, `ask.ts:448`) with `buildHarnessEnv` (`ask.ts:356`), under timeout + `killTree` (`ask.ts:866`) and an output cap (`ask.ts:856`). An absent tool omits **only** its own metric facts, reported not guessed (N5). External tools are behind an opt-in env gate `KIP_INDEX_TOOLS` (mirroring `KIP_ASK_LIVE`). Determinism: output = f(git HEAD sha, present-tool set).

**Consequences.** The guaranteed tier always produces a byte-stable core scan on any platform with no spawns; the accelerator tier adds metrics only when its tool is actually present, and the resulting graph is a pure function of the HEAD sha and the resolved tool set — a machine without `tokei` simply has no `tokei`-derived facts, never fabricated ones.

**Rejected alternatives.** Assuming bash/coreutils are present (the miner runs on Windows); hard-requiring `rg`/`tokei` (must degrade, not fail); estimating a metric when its tool is absent (N5 — emit nothing and surface the skip).

*Traceability: docs/60 N5; ADR-B6 (zero-new-dependency / Windows-lockfile policy); ADR-B8 (probe-decides-skip / everything-after-probe-decides-pass); packages/kip-sdk/src/cli/ask.ts:221/:356/:448/:547/:856/:866.*
