...
export async function runPlanProcessPhase(args: import("./phaseTypes").RunPlanProcessPhaseArgs): Promise<ProcessDefinitionReport> {
  ...
  if (isRawTextSession) {
    ...
    processDefinitionSystemPrompt = [
      ...
      "- Uses `agent` tasks for generating content (planning, drafting, analysis)",
      "- Does NOT use `node` kind effects",
      "",
      "IMPORTANT: The agent executor can only return text — it CANNOT write files or run commands.",
      "If the task requires writing a file, you MUST write it yourself in the process body using",
      "Node.js fs: `import { writeFile, mkdir } from 'node:fs/promises'`.",
      "Capture the agent task's text output and write it to the required path with writeFile().",
      "Always mkdir the parent directory first with { recursive: true }.",
      "",
