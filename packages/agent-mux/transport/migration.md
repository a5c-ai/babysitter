...
`packages/agent-mux/transport` is the published transport/proxy runtime seam used by the agent-mux release and staging pipelines. `amux launch` imports it as part of the public install chain.
...
- `@a5c-ai/agent-mux-transport` is a public package in the agent-mux family.
...
```bash
npm run build --workspace=@a5c-ai/agent-mux-transport
npm run test --workspace=@a5c-ai/agent-mux-transport
npm run scorecard:migration --workspace=@a5c-ai/agent-mux-transport
```
...
| --- | --- | --- | --- |
| Public package policy | `packages/agent-mux/transport/package.json` plus `npm run scorecard:migration`. | The package is publishable, metadata-complete, and aligned with the public docs map. | Green: package metadata describes a public artifact with an auditable publish surface. |
| Workspace seam health | `npm run build` and `npm run test` under `@a5c-ai/agent-mux-transport`. | Published consumers can still build against the seam confidently. | Green: the runtime seam compiles and tests cleanly. |
| Launcher/runtime seam | `packages/agent-mux/cli/src/commands/launch.ts` and `packages/agent-mux/transport/src/index.ts`. | The launcher imports the runtime seam that the published package actually ships. | Green: workspace integration and package exports describe the same runtime truth. |
| Docs honesty | `packages/agent-mux/transport/README.md`, `packages/agent-mux/transport/architecture.md`, and this file. | Operators can tell, from package docs alone, that `transport-mux` is part of the public runtime chain. | Green: docs describe one active runtime seam and its remaining migration boundaries. |
...
- `packages/agent-mux/transport/package.json` remains publishable and aligned with the package docs map.
...
- `packages/agent-mux/transport/README.md` describes this workspace as the published runtime seam.
- `packages/agent-mux/transport/architecture.md` remains the design reference for the runtime boundary.
