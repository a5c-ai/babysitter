...
export function buildProcessDefinitionSystemPrompt(
  ...
  return [
    ...
    "- In non-interactive mode, skip user questions but still parse the request, inspect the repo/workspace structure, and search the local babysitter process library for the most relevant specialization or methodology before authoring the process.",
    ...
    "- You may inspect local babysitter process references when they materially improve the process design. Prefer project `.a5c/processes/`, local plugin paths such as `plugins/babysitter/skills/babysit/process/`, repository `library/` materials, and local babysitter discover/profile CLI commands when available.",
