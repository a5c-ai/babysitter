...
const COMMANDS: CommandInfo[] = [
  ...
  // Session commands
  { name: "session:init", description: "Initialize a session", category: "session", tags: ["init", "start", "session"], relatedCommands: ["session:associate", "session:resume"] },
  { name: "session:state", description: "Show session state", category: "session", tags: ["state", "inspect", "session"] },
  { name: "session:update", description: "Update session state", category: "session", tags: ["update", "session"] },
  // Harness commands
  { name: "harness:discover", description: "Discover available harness CLIs", category: "harness", tags: ["discover", "list", "harness"], example: "babysitter harness:discover --json" },
  { name: "harness:call", description: "Create and run a programmatic session", category: "harness", tags: ["call", "create", "run", "harness"], example: "babysitter harness:call --harness claude-code --process ./process.js#process" },
  { name: "harness:resume", description: "Resume an existing run", category: "harness", tags: ["resume", "continue", "harness"], example: "babysitter harness:resume --run-id <runId>" },
  { name: "harness:observe", description: "Launch live observer dashboard", category: "harness", tags: ["observe", "dashboard", "live", "ui"] },
  // Plugin commands
  ...
  { name: "session:associate", description: "Associate session with a run", category: "session", tags: ["associate", "bind", "session"] },
  { name: "session:resume", description: "Resume a session", category: "session", tags: ["resume", "continue", "session"] },
  { name: "session:check-iteration", description: "Check if iteration should continue", category: "session", tags: ["check", "iteration", "session"] },
  { name: "session:last-message", description: "Get last message in session", category: "session", tags: ["last", "message", "session"] },
  { name: "session:iteration-message", description: "Get iteration message", category: "session", tags: ["iteration", "message", "session"] },
  // Skill commands
  ...
  // Harness aliases
  { name: "harness:yolo", description: "Non-interactive harness run", category: "harness", tags: ["yolo", "non-interactive", "run", "harness"] },
  { name: "harness:plan", description: "Plan-only harness run (stops after phase 1)", category: "harness", tags: ["plan", "harness", "phase"] },
  { name: "harness:forever", description: "Infinite-loop harness run", category: "harness", tags: ["forever", "infinite", "loop", "harness"] },
  { name: "harness:retrospect", description: "Analyze past runs for insights", category: "harness", tags: ["retrospect", "analyze", "insights", "harness"] },
  { name: "harness:cleanup", description: "Clean up old runs and artifacts", category: "harness", tags: ["cleanup", "clean", "artifacts", "harness"] },
  { name: "harness:assimilate", description: "Assimilate external methodology", category: "harness", tags: ["assimilate", "methodology", "harness"] },
  { name: "harness:doctor", description: "Diagnose run health", category: "harness", tags: ["doctor", "diagnose", "health", "harness"] },
  { name: "harness:contrib", description: "Submit feedback or contributions", category: "harness", tags: ["contrib", "feedback", "contributions", "harness"] },
  { name: "harness:help", description: "Show harness help", category: "harness", tags: ["help", "harness"] },
  { name: "harness:user-install", description: "Set up babysitter for current user", category: "harness", tags: ["install", "user", "setup", "harness"] },
  { name: "harness:project-install", description: "Set up babysitter for current project", category: "harness", tags: ["install", "project", "setup", "harness"] },
  ...
  // MCP
  { name: "mcp:serve", description: "Start MCP server", category: "mcp", tags: ["mcp", "server", "serve"] },
  // Misc
...
export function getContextualSuggestions(context: {
  ...
  if (!context.hasRun) {
    suggestions.push(...COMMANDS.filter((c) =>
      c.name === "harness:call" || c.name === "harness:discover",
    ));
  }
