...
CRITICAL RULE: If a run has a `PROCESS_RUNTIME_ERROR`, recover with
`babysitter run:recover-process-error <runDir>` after fixing or patching the
underlying bad process/task artifact. Manual journal edits are a last-resort
for corruption the targeted CLI cannot handle.
