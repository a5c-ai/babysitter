...
[CLEANUP MODE] Clean up the resolved runs directory (default `~/.a5c/runs`, with repo-local compatibility reads) and `.a5c/processes`. Aggregates insights from completed/failed runs into docs/run-history-insights.md, then removes old run data and orphaned process files.

Create and run a cleanup process that:
- Scans the resolved runs directory for completed/failed runs
...
- The process scans the resolved runs directory for completed/failed runs, aggregates insights, writes summaries, then removes old data
